...
function personalizeMissionForDashboardTask(mission, task) {
  ...
  const sourceTask = (selectedTask && selectedTask.id ? selectedTask : null) || (task && task.id ? task : null) || (mission && mission.id ? mission : null) || defaultTaskForCurrentRole();
...
function getMissionConfigByKey(key) {
  ...
  if (selectedTask && selectedTask.id) {
    return personalizeMissionForDashboardTask(selectedTask, selectedTask);
  }
  ...
  const taskDef = findTaskDefinitionById(normalizedKey) || defaultTaskForCurrentRole();
  return personalizeMissionForDashboardTask(taskDef, taskDef);
}
...
function selectedMissionKey() {
  ...
  return stored || "ai-resume-screener";
}
...
function activeTaskId() {
  ...
  return state.mission ? state.mission.taskId : "ai-resume-screener";
}
...
function navigateToResults() {
  ...
  try {
    if (window.parent && window.parent !== window) {
      window.parent.location.href = "/results";
      return;
    }
  } catch (e) {}
...
function renderMission() {
  ...
  const demoPillLabel = document.querySelector(".demo-pill span");
  if (demoPillLabel) {
    demoPillLabel.textContent = "TIME REMAINING";
  }
